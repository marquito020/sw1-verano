...
import {
  ...
  useDeleteEvent,
} from "../../../../hooks/useEvent.hook";
import { useSelector } from "react-redux";
import { RootState } from "../../../../redux/store";
import { eventsUrl } from "../../../../services/event.service";

function OrganizerEventList() {
  const navigate = useNavigate();
  const { mutate } = useSWRConfig();
  const user = useSelector((state: RootState) => state.user);
  const { events, isLoading, error } = useAllEventsOrganizer(
    user.organizer ? user.organizer.id : 0
  );
          ...
          >
            ...
            <div className="md:flex md:justify-center gap-3 md:p-2 xl:mb-0 sm:mb-2 mr-2">
              ...
              <button
                onClick={() => {
                  navigate(
                    `${PrivateRoutes.PRIVATE}${PrivateRoutes.EVENTS}/show/${event.id}`
                  );
                }}
              ...
              <button
                onClick={() => {
                  toast((t) => (
                    <div>
                      <h3>Esta seguro de eliminar el evento?</h3>
                      <br />
