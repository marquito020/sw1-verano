...
function UpdateOrganizerForm() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { organizerFound } = useGetOrganizer(parseInt(`${id}`));
  const { updateOrganizer } = useUpdateOrganizer(parseInt(`${id}`));
  ...
  const handleSubmit = async (e: FormEvent<OrganizerCustomForm>) => {
    ...
    try {
      const response = await updateOrganizer(formData);
      if (response?.id) {
        setMessageError("");
        navigate(`${PrivateRoutes.PRIVATE}${PrivateRoutes.ORGANIZERS}`);
      ...
      <div className="bg-white p-10 mt-16 xl:ml-5 rounded-lg shadow-lg w-full">
        <div className="px-4 sm:px-0">
          ...
          <img
            ...
            src={organizerFound?.user.imageSecureUrl}
            alt={organizerFound?.user.name}
          />
        ...
        <form onSubmit={handleSubmit} className="mt-6 border-t border-gray-100">
          <dl className="divide-y divide-gray-100">
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <input
                defaultValue={organizerFound?.user.name}
                id="name"
                name="name"
                type="text"
            ...
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <input
                defaultValue={organizerFound?.user.email}
                id="email"
                name="email"
                type="email"
                className="mt-1 py-1 px-2 outline-none border border-gray-500 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"
            ...
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <input
                required={true}
                defaultValue={organizerFound?.user.phone}
                id="phone"
                name="phone"
                type="text"
                className="mt-1 py-1 px-2 outline-none border border-gray-500 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"
            ...
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <input
                defaultValue={organizerFound?.user.address}
                id="address"
                name="address"
                type="text"
                className="mt-1 py-1 px-2 outline-none border border-gray-500 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"
            ...
            <div className="px-4 py-6 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
              ...
              <input
                defaultValue={organizerFound?.webSite}
                id="webSite"
                name="webSite"
                type="text"
                className="mt-1 py-1 px-2 outline-none border border-gray-500 text-sm leading-6 text-gray-700 sm:col-span-2 sm:mt-0"
            ...
            <div className="gap-2 text-white">
              ...
              <button
                onClick={() =>
                  navigate(
                    `${PrivateRoutes.PRIVATE}${PrivateRoutes.ORGANIZERS}`,
                    { replace: true }
                  )
                }
