...
import { PhotographerCustomForm } from "../../../../interfaces/photographer.interface";
import { useAddPhotographer } from "../../../../hooks/usePhotographer.hook";

function AddPhotographerForm() {
  ...
  const handleSubmit = async (e: FormEvent<PhotographerCustomForm>) => {
    ...
    const toastId = toast.loading("Creando nuevo fotografo...");
    try {
      const response = await addPhotographer(formData);
      if (response?.id) {
        setMessageError("");
        setIsOpen(false);
        toast.success("Fotografo creado exitosamente", { id: toastId });
      } else if (response && "message" in response) {
        setMessageError(response.message as string);
        toast.error("Error al crear al nuevo fotografo", { id: toastId });
      } else {
        toast.error("Error al crear al nuevo fotografo", { id: toastId });
      }
    } catch (err) {
